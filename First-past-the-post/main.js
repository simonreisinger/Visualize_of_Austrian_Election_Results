...
const REGION_TYPES = [
  REGION_TYPE_MUNICIPALITY,
  REGION_TYPE_COUNTY,
  REGION_TYPE_ELECTORALDISTRICT,
];
...
function main() {
  ...
  d3.dsv(";", "./data/NRW19.csv").then((data) => {
    ...
    let barFptpG = bar(
      data.municipalities.reduced.mostVotedParty,
      "#svg_bar_fptp",
      d3.select("#FPTPdiv"),
      mainBarChartArea
    );
    ...
    tooltipBarChartDivSecondary = tooltip
      .append("div")
      .style("display", "none");
    let tooltipBarElements = bar(
      null,
      "#bar_tooltip",
      tooltipBarChartDiv,
      tooltipBarChartArea
    ...
    yearSelect.on("change", () =>
      main_selectionChangeFun(yearDataMap, barPlots)
    );

    let regionSelect = d3.select("#region_select");
    for (let regionType of REGION_TYPES) {
      regionSelect.append("option").attr("value", regionType).html(regionType);
    }
    ...
    d3.dsv(";", "./data/NRW17.csv").then((data) => {
      ...
      d3.dsv(";", "./data/NRW13.csv").then((data) => {
        ...
        d3.selectAll(".pc_master_div").style("width", pcArea + "px");
...
function main_addYear(year) {
  d3.select("#year_select").append("option").attr("value", year).html(year);
}
...
function main_updateData(newData, yearDataMap, regionType, year, barPlots) {
  ...
  // PIE CHARTS
  updatePieChart(
    newData.importantFPTP[regionType.replace(" ", "")],
    { imp: "green", notimp: "grey", notimpX: "lightgrey" },
    "#svg_pie_Suppressed_fptp"
  );
  updatePieChart(
    newData.importantPR,
    { imp: "green", notimp: "grey" },
    "#svg_pie_Suppressed_pr"
  );
  ...
  bar_update(
    ...
    barPlots[0].labels,
    mainBarChartArea,
    newData.reduced.mostVotedParty,
    1000
  ); // TODO
