...
function data_initialize(data, year) {
  ...
  if (DEBUG && Object.keys(counties).length !== 116) {
    ...
    console.error(
      year + " counties length was " + len + " but it should be 116"
    );
  }
  counties = data_preprocessRegions(counties);

  // Add electoral district to Counties

  let countiesElectoralDistrict = data.filter(function (value) {
    let gkz = value.GKZ;
  ...
  for (let i in countiesElectoralDistrict) {
    let new_GKZ = countiesElectoralDistrict[i].GKZ.substring(4, 1) + "00";
    for (let party in countiesElectoralDistrict[i]) {
      if (!notParty.includes(party)) {
        counties[new_GKZ][party] = countiesElectoralDistrict[i][party];
      }
    }
  }
  ...
  let resultsByElectoralDistricts = firstPassThePostElectoralDistrict(
    lol,
    year
  );
  electoralDistrictDataSet[year] = firstPassThePost(
    resultsByElectoralDistricts,
    year
  );
  ...
  electoralDistricts.reduced.percentages.mostVotedParty = data_calculatePercentageFromMostVotedParty(
    mostVotedParty
  );
  ...
  importantFPTP.Municipalities = findTooManyVotersOfDistrict(
    municipalities,
    margin
  );
  //importantFPTP.Districts = findTooManyVotersOf(counties, margin); // TODO einblenden
  importantFPTP.Electoraldistrict = findTooManyVotersOfElectoralDistrict(
    resultsByElectoralDistricts,
    margin + 1
  ); // TODO edit here
...
function findTooManyVotersOfElectoralDistrict(voting_data, margin) {
  ...
  for (let i in voting_data) {
    ...
    for (let party in loosingParties) {
      if (
        list[loosingParties[party]] !== null &&
        list[loosingParties[party]] !== undefined &&
        party !== "invalid"
      ) {
        allVotes += convertStringToNumber(list[loosingParties[party]]);
        invalidVotes_LoosingParties += convertStringToNumber(
          list[loosingParties[party]]
        );
      }
    }
  ...
  return {
    imp: validVotes,
    notimp: invalidVotes_WinningParty,
    notimpX: invalidVotes_LoosingParties,
  };
}

function findTooManyVotersOfDistrict(voting_data, margin) {
  let validVotes = 0;
  let invalidVotes_WinningParty = 0;
  let invalidVotes_LoosingParties = 0;
  let allVotes = 0;
  for (let i in voting_data) {
    if (!Number.isNaN(parseInt(i))) {
      let list = voting_data[i].partiesAll;
      let loosingParties = Object.keys(list).sort(function (a, b) {
        return (list[a] === null) - (list[b] === null) || list[b] - list[a];
      });
      loosingParties.shift();
      let secondPartyVotes = list[loosingParties[0]];
      let winningPartyVotes =
        voting_data[i].partiesAll[voting_data[i].mostVotedParty];
      allVotes += winningPartyVotes;
      let diff = winningPartyVotes - (secondPartyVotes + margin);
      invalidVotes_WinningParty += diff > 0 ? diff : 0;
      for (let party in loosingParties) {
        if (
          ...
          invalidVotes_LoosingParties += list[loosingParties[party]];
        }
      }
      validVotes += secondPartyVotes + margin; //  TODO think if this is right
    }
...
function VierProzentHuerde(results) {
  ...
  var unvalidVotes = 0;
  var validVotes = 0;
  var totalVotes = convertStringToNumber(results.Abgegebene);
  for (var i in results) {
    // console.log(i);
    if (!notParty.includes(i)) {
      var currentPartyVotes = convertStringToNumber(results[i]);
      if ((100.0 * currentPartyVotes) / totalVotes < 4.0) {
        unvalidVotes += convertStringToNumber(results[i]);
      } else {
        validVotes += convertStringToNumber(results[i]);
      }
    } else {
      //console.log(i);
    }
  ...
  for (let p in _partyColors) {
    if (NRParties[year].includes(p)) {
      if (!partyNames.includes(p)) {
        if (
          selectedParties[SONST] === null ||
          selectedParties[SONST] === undefined
        ) {
          selectedParties[SONST] = 0;
        }
        selectedParties[SONST] += Math.round(
          (parseFloat(nationalResults[p].replace(/\./g, "")) /
            sumOfValidVotes) *
            183.0
        );
      } else {
        selectedParties[p] = Math.round(
          (parseFloat(nationalResults[p].replace(/\./g, "")) /
            sumOfValidVotes) *
            183.0
        );
...
function data_formatVotes(votes) {
  return votes === undefined
    ...
    : votes === ""
    ? 0
    : parseInt(votes.replace(".", "").replace(".", ""));
}
...
function data_preprocessRegions(manyRegions) {
  ...
  for (var region of manyRegions) {
    ...
    value.mostVotedParty = Object.keys(Object.assign(value.partiesAll)).reduce(
      function (keyA, keyB) {
...
function data_getVotesTotal(region) {
  if (region.Abgegebene !== undefined)
    return data_formatVotes(region.Abgegebene);
  if (region.Gültige !== undefined && region.Ungültige !== undefined)
    return (
      data_formatVotes(region.Gültige) + data_formatVotes(region.Ungültige)
    );
}
...
function data_reduceRegions(manyPreprocessedRegions) {
  ...
  for (let iso in manyPreprocessedRegions) {
    ...
    if (percentages.mostVotedParty[mvp] === undefined)
      percentages.mostVotedParty[mvp] = 0;
    mostVotedParty[mvp]++;

    if (!partyNames.includes(mvp)) {
      if (mostVotedParty[SONST] === undefined) mostVotedParty[SONST] = 0;
      if (percentages.mostVotedParty[SONST] === undefined)
        percentages.mostVotedParty[SONST] = 0;
...
function getDatasetByISO(data, iso, year) {
  ...
  if (year === 2013) {
    ...
    if (iso === 41301 || iso === 41335)
      return getDataByISO(data, 41346, 41346, null);
    // Mit 1. Jänner 2019 wurde die Gemeinde Ahorn nach Helfenberg eingemeindet
    if (iso === 41302 || iso === 41310)
      return getDataByISO(data, 41345, 41345, null);
    ...
    if (iso === 41340 || iso === 41625)
      return getDataByISO(data, 41628, 41628, null);
    // Peuerbach mit den Nachbargemeinden Bruck-Waasen
    if (iso === 40803 || iso === 40819)
      return getDataByISO(data, 40835, 40835, null);
  }
...
function calculateBarData(year) {
  ...
  for (let currentWahlkreis in electoralDistrictDataSet[year]) {
    if (
      wkm[electoralDistrictDataSet[year][currentWahlkreis].party] ===
        undefined ||
      wkm[electoralDistrictDataSet[year][currentWahlkreis].party] === null
    ) {
      wkm[electoralDistrictDataSet[year][currentWahlkreis].party] = 0;
    }
    for (let currentCounty in electoralDistrictMandate[year]) {
      if (currentCounty === currentWahlkreis) {
        if (
          partyNames.includes(
            electoralDistrictDataSet[year][currentWahlkreis].party
          )
        ) {
          wkm[electoralDistrictDataSet[year][currentWahlkreis].party] +=
            electoralDistrictMandate[year][currentCounty].Mandate;
        } else {
          wkm["SONST"] += electoralDistrictMandate[year][currentCounty].Mandate;
        }
        break;
      }
    }
...
function firstPassThePostElectoralDistrict(dataset, year) {
  electoralDistrictMandate[year] = [];
  let mostVotes = [];
  for (let selectedState in dataset) {
    let xxxx = data_parseGebietsname(
      removeNonASCIICharacters(dataset[selectedState].Gebietsname)
    );
    ...
    let electoralDistrict =
      districtsByEelectoralDistrict[xxxx].ElectoralDistrict;
    if (
      mostVotes[electoralDistrict] === null ||
      mostVotes[electoralDistrict] === undefined
    ) {
      mostVotes[electoralDistrict] = {};
      mostVotes[electoralDistrict].Gebietsname = electoralDistrict;
      electoralDistrictMandate[year][electoralDistrict] = {};
      electoralDistrictMandate[year][
        electoralDistrict
      ].Gebietsname = electoralDistrict;
      electoralDistrictMandate[year][electoralDistrict].Mandate =
        districtsByEelectoralDistrict[
          removeNonASCIICharacters(dataset[selectedState].Gebietsname)
        ].Mandate;
      for (let parties in NRParties[year]) {
        if (parties !== "SONST") {
          let currentPartyVotes = dataset[selectedState][
            NRParties[year][parties]
          ].replace(/\./g, "");
          mostVotes[electoralDistrict][
            NRParties[year][parties]
          ] = currentPartyVotes;
      ...
      for (let parties in NRParties[year]) {
        if (parties !== "SONST") {
          let currentPartyVotes = parseInt(
            dataset[selectedState][NRParties[year][parties]].replace(/\./g, "")
          );
          mostVotes[electoralDistrict][NRParties[year][parties]] = (
            parseInt(mostVotes[electoralDistrict][NRParties[year][parties]]) +
            currentPartyVotes
          ).toString();
        }
      }
  ...
  for (let selectedState in dataset) {
    ...
    for (let parties in NRParties[year]) {
      if (parties !== "SONST") {
        if (
          DEBUG &&
          dataset[selectedState][NRParties[year][parties]] === undefined
        ) {
        ...
        if (
          currentPartyVotes >=
          mostVotes[dataset[selectedState].Gebietsname].votes
        ) {
